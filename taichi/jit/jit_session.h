...
public:
  ...
  virtual void *lookup(const std::string Name) { TI_NOT_IMPLEMENTED }

  virtual llvm::DataLayout get_data_layout() = 0;

  static std::unique_ptr<JITSession>
  create(TaichiLLVMContext *tlctx, const CompileConfig &config, Arch arch);
