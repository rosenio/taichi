...
bool cfg_optimization(
    IRNode *root, bool after_lower_access, bool autodiff_enabled,
...
void make_block_local(IRNode *root, const CompileConfig &config,
                      const MakeBlockLocalPass::Args &args);
void make_cpu_multithreaded_range_for(IRNode *root,
                                      const CompileConfig &config);
void make_mesh_thread_local(IRNode *root, const CompileConfig &config,
                            const MakeBlockLocalPass::Args &args);
void make_mesh_block_local(IRNode *root, const CompileConfig &config,
                           const MakeMeshBlockLocal::Args &args);
void demote_mesh_statements(IRNode *root, const CompileConfig &config,
                            const DemoteMeshStatements::Args &args);
bool remove_loop_unique(IRNode *root);
bool remove_range_assumption(IRNode *root);
bool lower_access(IRNode *root, const CompileConfig &config,
                  const LowerAccessPass::Args &args);
void auto_diff(IRNode *root, const CompileConfig &config,
               AutodiffMode autodiffMode, bool use_stack = false);
/**
 * Check whether the kernel obeys the autodiff limitation e.g., gloabl data
 * access rule
 */
void differentiation_validation_check(IRNode *root, const CompileConfig &config,
...
bool transform_statements(
    IRNode *root, std::function<bool(Stmt *)> filter,
 ...
 */
bool replace_statements(IRNode *root, std::function<bool(Stmt *)> filter,
...
void optimize_bit_struct_stores(IRNode *root, const CompileConfig &config,
...
void compile_to_offloads(IRNode *ir, const CompileConfig &config,
                         const Kernel *kernel, bool verbose,
                         AutodiffMode autodiff_mode, bool ad_use_stack,
                         bool start_from_ast);

void offload_to_executable(IRNode *ir, const CompileConfig &config,
                           const Kernel *kernel, bool verbose,
                           bool determine_ad_stack_size,
                           bool lower_global_access, bool make_thread_local,
...
void compile_to_executable(IRNode *ir, const CompileConfig &config,
                           const Kernel *kernel, AutodiffMode autodiff_mode,
                           bool ad_use_stack, bool verbose,
...
void compile_function(IRNode *ir, const CompileConfig &config, Function *func,
                      AutodiffMode autodiff_mode, bool verbose,
