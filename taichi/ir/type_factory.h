...
public:
  ...
  const Type *
  get_struct_type(const std::vector<AbstractDictionaryMember> &elements,
                  const std::string &layout = "none");

  const Type *
  get_argpack_type(const std::vector<AbstractDictionaryMember> &elements,
                   const std::string &layout = "none");

  const Type *
  get_struct_type_for_argpack_ptr(DataType dt,
                                  const std::string &layout = "none");

  const Type *get_ndarray_struct_type(DataType dt, int ndim,
  ...
  Type *get_quant_fixed_type(Type *digits_type, Type *compute_type,
                             float64 scale);

  Type *get_quant_float_type(Type *digits_type, Type *exponent_type,
                             Type *compute_type);

  BitStructType *get_bit_struct_type(
      PrimitiveType *physical_type, const std::vector<Type *> &member_types,
  ...
  Type *get_quant_array_type(PrimitiveType *physical_type, Type *element_type,
                             int num_elements);
...
private:
  ...
  std::unordered_map<std::pair<std::string, Type *>, std::unique_ptr<Type>,
  ...
  // TODO: is_bit_ptr?
  std::unordered_map<std::pair<const Type *, bool>, std::unique_ptr<Type>,
                     hashing::Hasher<std::pair<const Type *, bool>>>
      pointer_types_;
  std::mutex pointer_mut_;

  std::unordered_map<std::tuple<int, bool, Type *>, std::unique_ptr<Type>,
                     hashing::Hasher<std::tuple<int, bool, Type *>>>
      quant_int_types_;
  std::mutex quant_int_mut_;

  std::unordered_map<std::tuple<Type *, Type *, float64>, std::unique_ptr<Type>,
                     hashing::Hasher<std::tuple<Type *, Type *, float64>>>
      quant_fixed_types_;
  std::mutex quant_fixed_mut_;

  std::unordered_map<std::tuple<Type *, Type *, Type *>, std::unique_ptr<Type>,
