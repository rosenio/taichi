...
public:
  size_t operator()(T const &val) const { return std::hash<T>{}(val); }
};

namespace {
template <typename T> inline size_t hash_value(T const &value) {
  return Hasher<T>{}(value);
}

template <typename T> inline void hash_combine(size_t &seed, T const &value) {
  // Reference:
...
namespace {
template <int N, typename... Ts> struct TupleHasher {
