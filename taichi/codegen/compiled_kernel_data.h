...
public:
  void set_launch_id(int id) { launch_id_ = id; }

  int get_launch_id() const { return launch_id_; }
...
public:
  ...
  CompiledKernelDataFile() { std::copy(kHeadStr, kHeadStr + kHeadSize, head_); }

  void set_arch(Arch arch) { arch_ = arch; }

  void set_metadata(std::string metadata) { metadata_ = std::move(metadata); }

  void set_src_code(std::string src) { src_code_ = std::move(src); }

  const Arch &arch() const { return arch_; }

  const std::string &metadata() const { return metadata_; }

  const std::string &src_code() const { return src_code_; }
...
public:
  ...
  virtual Err debug_print(std::ostream &os) const { return dump(os); }

  virtual Err check() const { return Err::kNoError; }
