...
template <typename T> T ceil_div(T v, T div) {
...
public:
  ...
  bool equals(const Polymorphic *other) const { return (*this) == (*other); }
...
public:
  ...
  void set_debug_name(const std::string &s) { debug_name_ = s; }

  virtual NodeRefs incoming() const { return {}; }

  virtual NodeRefs outgoing() const { return {}; }

  virtual bool is_leaf() const { return false; }

  virtual bool is_tree_node() const { return false; }

private:
  bool is_equal(const Polymorphic &other) const override { return false; }
...
public:
  ...
  void register_elem_offset(const MemRefAggregateTypeInterface *t, int n,
                            size_t offset) {
...
class Block {
public:
  template <typename T, class... E> T *emplace_back(E... args) {
    nodes_.push_back(std::make_unique<T>(args...));
    return static_cast<T *>(nodes_.back().get());
  }

  template <typename T> T *push_back(std::unique_ptr<T> &&val) {
    T *ptr = val.get();
