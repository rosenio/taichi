...
public:
  ...
  int get_num_elements() const override { return elements_.size(); }
...
public:
  ...
  const tinyir::Type *element_type() const override { return element_type_; }

  bool is_constant_shape() const override { return true; }
...
public:
  ...
  std::vector<size_t> get_constant_shape() const override { return {size_}; }
...
const tinyir::Type *translate_ti_type(tinyir::Block &ir_module,
                                      const DataType t, bool has_buffer_ptr);
...
std::unordered_map<const tinyir::Node *, uint32_t>
ir_translate_to_spirv(const tinyir::Block *blk,
