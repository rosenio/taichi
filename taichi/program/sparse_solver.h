...
public:
  ...
  template <typename T> T solve(const T &b);

  template <typename T, typename V>
  void solve_rf(Program *prog, const SparseMatrix &sm, const Ndarray &b,
                const Ndarray &x);
  bool info() override;
};
...
public:
  ...
  bool compute(const SparseMatrix &sm) override { TI_NOT_IMPLEMENTED; };
  void analyze_pattern(const SparseMatrix &sm) override;
  ...
  bool info() override { TI_NOT_IMPLEMENTED; };
...
private:
  ...
  void solve_cholesky(Program *prog, const SparseMatrix &sm, const Ndarray &b,
                      const Ndarray &x);
  void solve_lu(Program *prog, const SparseMatrix &sm, const Ndarray &b,
                const Ndarray &x);
};
...
std::unique_ptr<SparseSolver>
make_cusparse_solver(DataType dt, const std::string &solver_type,
                     const std::string &ordering);
} // namespace taichi::lang
