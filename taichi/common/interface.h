...
template <typename T> T *create_instance_raw(const std::string &alias);
...
template <typename T> std::vector<std::string> get_implementation_names();
...
#define TI_INTERFACE(T)                                                        \
  ...
  public:                                                                      \
    ...
    template <typename G> void insert(const std::string &alias) {              \
    ...
    template <typename G> void insert_new(const std::string &alias) {          \
      /*with ctor*/                                                            \
      implementation_factories.insert(                                         \
          std::make_pair(alias, [&]() { return std::make_shared<G>(); }));     \
      implementation_unique_factories.insert(                                  \
      ...
      implementation_unique_ctor_factories.insert(                             \
          std::make_pair(alias, [&](const Dict &config) {                      \
            return std::make_unique<G>(config);                                \
          }));                                                                 \
      implementation_raw_factories.insert(                                     \
          std::make_pair(alias, [&]() { return new G(); }));                   \
      implementation_placement_factories.insert(std::make_pair(                \
    ...
    template <typename G> void update(const std::string &alias) {              \
      if (has(alias)) {                                                        \
        remove(alias);                                                         \
      }                                                                        \
      insert<G>(alias);                                                        \
...
#define TI_INTERFACE_DEF(class_name, base_alias)                               \
  ...
  template <> class_name *create_instance_raw(const std::string &alias) {      \
    return TI_IMPLEMENTATION_HOLDER_NAME(class_name)::get_instance()           \
        ->create_raw(alias);                                                   \
  }                                                                            \
  template <>                                                                  \
