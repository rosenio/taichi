...
public:
  ...
  [[deprecated("create_aot_module")]] virtual TiAotModule
  load_aot_module(const char *module_path) {
...
template <typename THandle> struct devalloc_cast_t {
  ...
  static inline THandle
  devalloc2handle(Runtime &runtime, taichi::lang::DeviceAllocation devalloc) {
    return (THandle)((size_t)devalloc.alloc_id + 1);
  }
};

[[maybe_unused]] taichi::lang::DeviceAllocation
devmem2devalloc(Runtime &runtime, TiMemory devmem) {
  return devalloc_cast_t<TiMemory>::handle2devalloc(runtime, devmem);
}

[[maybe_unused]] TiMemory
devalloc2devmem(Runtime &runtime,
                const taichi::lang::DeviceAllocation &devalloc) {
  return devalloc_cast_t<TiMemory>::devalloc2handle(runtime, devalloc);
}

[[maybe_unused]] taichi::lang::DeviceAllocation
devimg2devalloc(Runtime &runtime, TiImage devimg) {
  return devalloc_cast_t<TiImage>::handle2devalloc(runtime, devimg);
}

[[maybe_unused]] TiImage
devalloc2devimg(Runtime &runtime,
                const taichi::lang::DeviceAllocation &devalloc) {
